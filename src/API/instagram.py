...
class Instagram:
    ...
    def create_media(image_url: str, caption="") -> str:
        ...
        try:
            response = requests.post(
                f"https://graph.facebook.com/{IG_API_VERSION}/{IG_USER_ID}/media?image_url={image_url}&caption={caption}&access_token={IG_ACCESS_TOKEN}"
            )
        ...
        try:
            ...
            logging.error(
                "error occurred while parsing instagram create media response"
            )
            logging.debug(e)
    ...
    def publish_media(media_id: str) -> str:
        ...
        try:
            response = requests.post(
                f"https://graph.facebook.com/{IG_API_VERSION}/{IG_USER_ID}/media_publish?creation_id={media_id}&access_token={IG_ACCESS_TOKEN}"
            )
            response.raise_for_status()
        except Exception as e:
        ...
        try:
            ...
            logging.error(
                "error occurred while parsing instagram publish media response."
            )
            logging.debug(e)
            logging.debug(response.text)
            exit()
